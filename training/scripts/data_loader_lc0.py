"""
Data loader for LC0-format 112-channel training data.
...
class LC0Dataset(IterableDataset):
    ...
    def __iter__(self) -> Iterator[Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]]:
        ...
        """
        rng = random.Random(self.seed + torch.utils.data.get_worker_info().id if torch.utils.data.get_worker_info() else self.seed)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python data_loader_lc0.py <data_dir>")
        sys.exit(1)
